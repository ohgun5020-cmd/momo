...
export const ElementWrapper = (): JSX.Element => {
  ...
  const [selectedContact, setSelectedContact] = useState("라라");
  const [isSelectOpen, setIsSelectOpen] = useState(false);
  const [inputValue, setInputValue] = useState("");
  const [keyboardOpen, setKeyboardOpen] = useState(true);
  const { messages, isReplying, sendMessage, clearConversation } = useChatConversation("라라");
  ...
  return (
    <main className="w-full min-h-screen bg-[#120520] flex justify-center" data-model-id="8002:1724" role="main">
      <div className="relative w-[375px] min-h-[812px] overflow-hidden bg-[#120520]">
        ...
        >
          <div className="inline-flex items-start gap-3 w-full">
            <img className="w-[25px] h-[25px] mt-0.5" alt="Lara avatar" src="/img/group-2147237152.png" />
        ...
        <form
          className={`absolute left-0 right-0 px-[18px] py-2 z-[10] transition-all duration-300 bg-[linear-gradient(180deg,rgba(18,5,32,0)_0%,rgba(18,5,32,0.58)_36%,rgba(18,5,32,0.82)_100%)] backdrop-blur-[2px] ${keyboardOpen ? "bottom-[300px]" : "bottom-[16px]"}`}
          onSubmit={handleSubmit}
          onClick={() => setKeyboardOpen(true)}
        >
          <div className="relative h-[38px]">
            <button type="button" className="absolute left-0 top-0 w-8 h-8 rounded-2xl bg-[#322c39] text-white text-[31px] leading-none" aria-label="Add">
              +
            </button>
        ...
        <section
          className={`absolute left-0 bottom-0 w-full h-[300px] bg-[#0f1013] z-[8] transition-transform duration-300 ${keyboardOpen ? "translate-y-0" : "translate-y-full"}`}

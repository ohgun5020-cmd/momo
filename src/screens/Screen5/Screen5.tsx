...
export const Screen5 = (): JSX.Element => {
  ...
  const [quickBubbleText, setQuickBubbleText] = useState("고민있오?");
  ...
  const handleQuickCardClick = (card: FeatureCard) => {
    ...
    setQuickBubbleText(card.subtitle);
    window.setTimeout(() => {
  ...
  return (
    <main className="w-full min-h-screen bg-[#120520] flex justify-center" data-model-id="8002:1596" role="main">
      <div className="relative w-[375px] min-h-[812px] overflow-hidden bg-[#120520]">
        ...
        {isSelectOpen && (
          <>
            ...
            <div className="absolute top-[100px] left-1/2 -translate-x-1/2 w-[262px] h-[94px] rounded-[14px] border border-[#4f4869] bg-[#241733e0] shadow-[0_12px_28px_rgba(0,0,0,0.35)] z-[6]">
        ...
        <Link
          to="/x7"
          className="absolute left-1/2 -translate-x-1/2 top-[414px] w-[63px] h-[25px] z-[4]"
          aria-label="Open chat"
        >
          <img className="w-full h-full" alt="MOMO logo" src="/img/vector-8.svg" />
        </Link>

        <section className="absolute left-4 bottom-[102px] z-[6] w-[359px] overflow-x-scroll overscroll-x-contain touch-pan-x [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <footer className="absolute bottom-0 left-0 w-full h-[90px] z-[7]">
          <div className="px-4 pt-[6px] flex items-center gap-3 text-white">
            ...
            <Link
              to="/x7"
              state={{ draft: quickBubbleText }}
              className="ml-1 flex-1 h-10 rounded-[22px] border border-[#4d4858] bg-[#180e22dd] px-4 flex items-center"
              aria-label="Open input"
            >
              <span
                className={`[font-family:'SF_Pro-Regular',Helvetica] text-[17px] leading-[22px] ${
                  selectedQuickCardId ? "font-bold text-white" : "font-normal text-[#ffffff4d]"
                }`}
              >
                {quickBubbleText}
              </span>
              <span className="ml-auto text-[#ffffff99]">
                <MicIcon />
              </span>
            </Link>
